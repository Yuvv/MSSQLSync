...
namespace SettingHelper {
  public partial class SettingHelper: Form {
    ...
    private void createTriggerAndTable(string database, string table) {
      ...
      _log.Info(string.Format("create trigger table for {0} done!", table));
      ...
      this.dbCmd.ExecuteNonQuery();
      _log.Info(string.Format("create trigger for {0} done!", table));
      ...
      this.dbCmd.ExecuteNonQuery();
      _log.Info(string.Format("insert existed data into {0} done!", table));
    }
    ...
    private void DBSync_Shown(object sender, EventArgs e) {
      ...
      reader.Close();
      _log.Info("Load setting OK!");

      Login loginForm = new Login();
      loginForm.ShowDialog(this);
      _log.Info("Database opened!");
      ...
      foreach (var table in this.syncTables) {
        ...
        item.SubItems.Add(table["database"].ToString());
    ...
    private void DBSync_FormClosing(object sender, FormClosingEventArgs e) {
			...
			sw.Close();
			_log.Info("settings write back done!");
      ...
      if (e.Data.GetDataPresent("System.Windows.Forms.TreeNode", false)) {
        ...
        if (reader.Read()) {
          ...
          this.syncTables.Add(new JObject(
            new JProperty("database", node.Parent.Text),
            new JProperty("name", node.Text),
          ...
          if (DialogResult.OK == MessageBox.Show(
            ...
            this.syncTables.Add(new JObject(
              new JProperty("database", node.Parent.Text),
              new JProperty("name", "tgrtable4" + node.Text),
    ...
    private void listViewAddedTable_KeyUp(object sender, KeyEventArgs e) {
      if (e.KeyCode == Keys.Delete) {
        foreach (ListViewItem item in this.listViewAddedTable.SelectedItems) {
          JObject t = null;
          foreach (var table in this.syncTables) {
            if (table["name"].ToString() == item.Text &&
              table["database"].ToString() == item.SubItems[1].Text) {
              t = (JObject)table;
              break;
            }
          }
          if (t != null) {
            this.syncTables.Remove(t);    // del sync table in setting
          }
          if (item.Text != item.SubItems[4].Text) {
            try {
              dbCmd.CommandText = string.Format("use [{0}]", item.SubItems[1].Text);
              ...
              dbCmd.ExecuteNonQuery();
              dbCmd.CommandText = string.Format(
                "if exists (select 1 from sysobjects where id=object_id('tgr4{0}') and objectproperty(id, 'istrigger')=1)" +
                "drop trigger [tgr4{0}];", item.SubItems[4].Text);
              dbCmd.ExecuteNonQuery();
